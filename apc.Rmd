...
ggplot(filter(forcing_only_apcs, apc_type=="absolute"), aes(x=apcs, y=ID, color=ID)) +
  geom_point(size=2) +
  ...
  ylab("") +
  ggtitle("|Change| in forcing required for phenology period start associated with each effect") +
  facet_grid(sex ~ .) +
...
ggplot(filter(days_only_apcs, apc_type=="absolute"), aes(x=apcs, y=as.factor(coldtohot), color=sex)) +
  ...
  geom_line(aes(x=errorbound, y=as.factor(coldtohot))) +
  geom_vline(xintercept=0) +
  theme_bw() +
  removeGrid(y=TRUE, x=FALSE) +
  facet_wrap(ID ~ . ) +
