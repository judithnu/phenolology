...
Simulate data from an ordered logistic distribution with three states and forcing values similar to mine. 
...
Eliminating the overall beta should help my model fit quite a lot. This makes it much harder to interpret parameters though. I can try a run that increases max_treedepth and  
...
## Which variables to include in slope?

I think clone and provenance only because they are biological.
...
mristoscaled$SPU_Name <- as.factor(mristoscaled$SPU_Name)
mristoscaled$Site <- as.factor(mristoscaled$Site)
mristoscaled$SPU_Name <- as.factor(mristoscaled$SPU_Name)
mristoscaled$Clone <- as.factor(mristoscaled$Clone)
mristoscaled$Year <- as.factor(mristoscaled$Year)
mristoscaled$Tree <- as.factor(mristoscaled$TreeID)

#fit prov as intercepts only
fit_provi <- bayespolr(state ~ sum_forcing + SPU_Name, data=mristoscaled)
fit_provs <- bayespolr(state ~ SPU_Name*sum_forcing, data=mristoscaled)
fit_provis <- bayespolr(state ~ SPU_Name*sum_forcing + SPU_Name, data=mristoscaled)
AIC(fitmristoscaled, fit_provi, fit_provs, fit_provis)

fit_sitei <- bayespolr(state ~ sum_forcing + Site, data=mristoscaled)
fit_sites <- bayespolr(state ~ Site*sum_forcing, data=mristoscaled)
fit_siteis <- bayespolr(state ~ Site*sum_forcing + Site, data=mristoscaled)
AIC(fitmristoscaled, fit_sitei, fit_sites, fit_siteis)

fit_clonei <- bayespolr(state ~ sum_forcing + Clone, data=mristoscaled)
fit_clones <- bayespolr(state ~ Clone*sum_forcing, data=mristoscaled)
#fit_cloneis <- bayespolr(state ~ Clone*sum_forcing + Clone, data=mristoscaled)
AIC(fitmristoscaled, fit_clonei, fit_clones, fit_cloneis)

fit_treei <- bayespolr(state ~ sum_forcing + TreeID, data=mristoscaled)
fit_trees <- bayespolr(state ~ TreeID*sum_forcing, data=mristoscaled)
fit_treeis <- bayespolr(state ~ TreeID*sum_forcing + TreeID, data=mristoscaled)
AIC(fitmristoscaled, fit_treei, fit_trees, fit_treeis)

fit_tc <- bayespolr(state ~ sum_forcing + Clone + TreeID, data=mristoscaled)
AIC(fitmristoscaled, fit_tc, fit_treei, fit_clonei)

fit_yr <- bayespolr(state ~ sum_forcing + Year, data=mristoscaled)
AIC(fitmristoscaled, fit_yr)


fit_all <- bayespolr(state ~ (SPU_Name + Site)*sum_forcing + Clone + Year, data=mristoscaled)


AIC(fitmristoscaled, fit_provis, fit_siteis, fit_clonei, fit_treei, fit_yr, fit_tc, fit_all)
```