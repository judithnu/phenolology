---
title: "No hierarchy ordered logit for PGTIS"
...
This document/script visualizes the relationship between heatsum and pollen phenophase for Prince George Tree Improvement Station and then models that relationship with an ordered logistic regression.

Phenology data was collected at Prince George Tree Improvement Station in three lodgepole pine orchards and provided to me by Chris Walsh and Rita Wagner.

Climate data is from a Prince George weather station - not the seed orchard.
...
## Model with PGTIS data from Rita Wagner only

I first considered only the PGTIS data from Rita Wagner
```{r Wagner only data processing}
...
### Wagner PGTIS and heatsum visualization 
```{r Wagner only visualization}
# Visualize the data
...
Parameters a1, a2, and b are the only parameters actually in the model. k, h1, and h2 are renamed and/or transformed parameters. 

- k describes the speed of transition. It is always between 0 and 1. Values close to 0 indicate a slow transition and values close to 1 indicate a rapid transition. 
- h1 is the daily heatsum (Celsius) when half of the trees have begun shedding pollen.
- h2 is the daily heatsum when half of the trees have finished shedding pollen.

Posterior distributions for these parameters with 50% shaded highest probability density intervals are below.

### Posterior distributions for model parameters
```{r Wagner only parameter posteriors}
dens(post_m1, show.HPDI=.5)
```

### Plot predicted data vs actual data
```{r Wagner only model vs data}
...
## All PGTIS data
Repeated the above with all data
```{r all PGTIS data}
...
## Ordered logit with levels
```{r Ordered logit with levels}
