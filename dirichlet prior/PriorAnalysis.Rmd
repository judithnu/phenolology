...
parplot <- function(pars) {
  cutplot <- mcmc_areas(pars, regex_pars="c.\\dmodel") +
    geom_vline(xintercept=c(pars$c.1true, pars$c.2true)) 
  
  betaplot <- mcmc_areas(pars, pars="betamodel") +
    geom_vline(xintercept=pars$betatrue) 
  ...
  allplot <- cowplot::plot_grid(cutplot, betaplot, h1plot, h2plot,
                                nrow=1, ncol=4, 
                                labels=paste("model", unique(pars$modelidtrue), 
                                             "rate=", unique(pars$beta_ratetrue), 
                                             "shape=", unique(pars$shapetrue)))
  print(allplot)
}
...
posterior_differencer <- function(pars) {
  c1_diff <- pars$c.1model - pars$c.1true 
  c2_diff <- pars$c.2model - pars$c.1true 
  h1_diff <- pars$h1model - h[1]
  h2_diff <- pars$h2model - h[2] 
  beta_diff <- pars$betamodel - pars$betatrue
...
diffplotter <- function(pars) {
  ...
  opars <- mcmc_areas(diffs, pars=c("h1_diff", "h2_diff", "beta_diff")) +
    xlim(c(-1,1))
  cowplot::plot_grid(cuts, opars, labels=paste("model", unique(pars$modelidtrue), 
                                             "rate=", unique(pars$beta_ratetrue), 
                                             "shape=", unique(pars$shapetrue)))
}
...
slowframe <- data.frame(beta = beta_slow, shape = slowshapes, beta_rate=beta_rate, c.1 = cutpoints_slow[1], c.2=cutpoints_slow[2])
fastframe <- data.frame(beta=beta_fast, shape= fastshapes, beta_rate=beta_rate, c.1 = cutpoints_fast[1], c.2=cutpoints_fast[2])
parframe <- rbind(slowframe, fastframe) %>%
  tidyr::expand(tidyr::nesting(beta, c.1, c.2, shape), beta_rate) 
...
# pgam <- parplot(names(paramsgam)[1], modelpars=paramsgam[[1]], truepars = parlist[[1]])
# pgam
...
label_names <- function(x, label) {
  colnames(x) <- paste0(colnames(x), label)
  return(x)
}
...
paramsgam <- map2(paramsgam, parlist[1:3], cbind)

#plot params and diffs
map(paramsgam, parplot)
map(paramsgam, diffplotter)

# calculate whether true value is in 50% hpdi
map(paramsgam, function(x) dplyr::select(x, ends_with("model"))))
```
