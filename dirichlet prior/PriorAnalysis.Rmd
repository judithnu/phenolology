...
parplot <- function(label, pardf, cutpoints, beta, h) {
  cutplot <- mcmc_areas(pardf, regex_pars="c") +
    geom_vline(xintercept=cutpoints) +
    xlim(c(cutpoints[1]-5,cutpoints[2]+5))
  betaplot <- mcmc_areas(pardf, pars="beta") +
    geom_vline(xintercept=beta) +
    xlim(c(beta-1,beta+1))
  h1plot <- mcmc_areas(pardf, pars = "h1") +
    geom_vline(xintercept=h[1])+
    xlim(c(h[1]-3,h[1]+3))
  h2plot <- mcmc_areas(pardf, pars = "h2") +
    geom_vline(xintercept=h[2])+
    xlim(c(h[2]-3,h[2]+3))
  plot_title <- paste(label, "|", "beta=", beta,"cutpoints=", cutpoints[1], ",", cutpoints[2])
  cowplot::plot_grid(cutplot, betaplot, h1plot, h2plot, nrow=1, ncol=4, labels=plot_title)
}
...
posterior_differencer <- function(model_params, true_params) {
  c1_diff <- model_params$c.1 - true_params$cut1 
  c2_diff <- model_params$c.2 - true_params$cut1 
  h1_diff <- model_params$h1 - h[1]
  h2_diff <- model_params$h2 - h[2] 
  beta_diff <- model_params$beta - true_params$beta
...
diffplotter <- function(model_params, true_params, model) {
  diffs <- posterior_differencer(model_params, true_params)
  ...
  opars <- mcmc_intervals(diffs, pars=c("h1_diff", "h2_diff", "beta_diff")) +
    xlim(c(-1,1))
  cowplot::plot_grid(cuts, opars, labels=model)
}
...
slowframe <- data.frame(beta = beta_slow, shape = slowshapes, beta_rate=beta_rate, cut1 = cutpoints_slow[1], cut2=cutpoints_slow[2])
fastframe <- data.frame(beta=beta_fast, shape= fastshapes, beta_rate=beta_rate, cut1 = cutpoints_fast[1], cut2=cutpoints_fast[2])
parframe <- rbind(slowframe, fastframe) %>%
  tidyr::expand(tidyr::nesting(beta, cut1, cut2, shape), beta_rate) 
parframe$model <- 1:nrow(parframe)
...
# make a cluster, leaving 2 cores free so my computer doesn't fall over when I check emails
...
# fit the models
gammafits <- parallel::parLapply(cl, parlist[1:3], function(x) {fit_gamma_model(simdatlist <- datlist, pars=x)})
...
paramsgam <- data.frame(rstan::extract(gammafits[[1]])) 

sum <- summary(paramsgam)

pgam <- parplot("gamma", paramsgam, cutpoints=c(parlist[[1]]$cut1, parlist[[1]]$cut2), beta=parlist[[1]]$beta, h=h)
pgam
