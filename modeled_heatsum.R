...
meantemp <- rnorm(100, 5, 10)
heat <- c()
for (i in 1:length(meantemp)) {
    if (meantemp[i] < 5) {
        heat[i]=0
        } else {
        heat[i] = meantemp[i]
...
ptrans <- 1/(1 + exp(-.2*(heatsum-150)))
phenophase <- c()

for (i in 1:length(ptrans)) {
    phenophase[i] = rbinom(1, size=1, prob=ptrans[i])
}

heatcalc <- function(x, Tb) {
    heat <- c()
        if (x < Tb) {
            heat=0
        } else {
...
sim <- data.frame(day = 1:100, meantemp = meantemp, phenophase = phenophase)
m_heat <- ulam(
  alist(
    #likelihood
    phenophase ~ dbinom(1,p),
    logit(p) <- beta*Heatsum + alpha,
    #covariate model
    Heat[day] <- heatcalc(meantemp, basetemp),
    Heatsum[day] <- cumsum(Heat[day]),
    #prior
    basetemp ~ normal(5,2),
    beta ~ dbeta(.5,5),
    alpha ~ normal(7,10)
  ...
  data = sim,
  warmup = 5, iter=10, chains = 1, cores = 1, algorithm="NUTS"
)
