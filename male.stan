...
parameters{
    ...
    vector[15] a_year;
    vector[259] a_clone;
    vector<lower=0, upper=1>[6] beta_prov;
...
//
//     for ( i in 1:10963 ) {
//         beta_tot[i] = beta + beta_prov[ProvenanceID[i]];
//     }
//
// }
model{
    ...
    //kappa_ep ~ exponential(.5);
   // kappa_diff ~ gamma(kappa_ep , 1 ); //prior on difference between cutpoints
    kappa ~ gamma(7.5 , 1 ); //nail down the first cutpoint


    // adaptive priors
    beta_prov ~ beta(2, prov_nu);
    ...
    //model
    for ( i in 1:10963 ) {
        phi[i] = a_prov[ProvenanceID[i]] + a_site[SiteID[i]] + a_year[YearID[i]] + a_clone[CloneID[i]] + (beta_prov[ProvenanceID[i]]) * sum_forcing[i];
    }
