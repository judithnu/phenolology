...
k <- rbeta(1,.5, 5)
eta <- k * phendf$forcing_accum
state <- rordlogit(length(eta), phi = eta, a = cutpoints)

sim <- data.frame(forcing = phendf$forcing_accum,state)

```

```{r visualize simulated_data}
hist(sim$state)
ggplot(sim, aes(x=forcing, y=state)) +
    geom_point()
```
