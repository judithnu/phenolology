...
femtest <- stan("phenology_corr.stan", chains = 1, warmup =20, iter=25, control=list(adapt_delta=0.9), data=fdf)
femfit <- stan("phenology_corr.stan",
                chains=8, cores=8, warmup=3000, iter=5000, data=fdf)

```

```{r save model, eval=FALSE}
saveRDS(femfit, file = "female_with_corr.rds")
posterior <- as.array(femfit)
post_re <- extract.samples(femfit)


h50 <- (post_re$kappa[,2] + post_re$a_site[,1] + post_re$a_prov[,1] + post_re$a_clone[,1])/(post_re$b_prov[,1]+post_re$b_site[,1] + beta)
hist(h50)
np <- nuts_params(femfit)
fempars <- precis(femfit, depth=3)
