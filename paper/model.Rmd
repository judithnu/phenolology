...
#phendf <- dplyr::select(phendf, -Site, -SPU_Name, -Sex, -TreeID, -Phenophase, -Date, -Clone, -daily_forcing_type)
...
fdf <- stancleaner(fdf)
#fdf <- select(fdf, Phenophase_Derived, forcing_accum, CloneID, ProvenanceID, SiteID, YearID, Tree)
...
fitfem <- stan("female_intercept_only.stan", 
               chains=1, cores=1, warmup = 10, iter=20, 
               data=fdf)
```
