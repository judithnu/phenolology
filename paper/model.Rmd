...
util$check_all_diagnostics(ffit.stan)
...
mcmc_pairs(post, np = np, regex_pars = c("kappa", "beta")) # show univariate histograms and bivariate scatter plots for selected parameters and is especially useful in identifying collinearity between variables (which manifests as narrow bivariate plots) as well as the presence of multiplicative non-identifiabilities (bananas). Each bivariate plot occurs twice and contains half the chains - so you can compare if chains produce similar results
mcmc_pairs(post, np = np, regex_pars = c("beta", "b_site"))
...
plot(post_re$beta, energy$Value)
title(paste(sex, "beta energy"))
...
rhats_good <- rhat(ffit.stan, pars=param_names[310:314])
...
mcmc_acf(post, lags = 10, regex_pars = c("beta", "kappa"))
