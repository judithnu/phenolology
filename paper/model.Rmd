...
sim <- rbind(g1, g2)
sim$state <- as.integer(sim$state)
sim$group <- as.integer(sim$group)
...
simfit <- ulam(
    alist(
        ...
        # model
        eta <- beta * forcing,
        cutpoints[1] ~ dnorm(alpha, 5),
        cutpoints[2] ~ dnorm(beta, 5),
        # priors
        beta ~ dbeta(.5,5)
        alpha ~
    ),
...
N = nrow(sim)
stan_rdump(c("N", "K", "forcing", "state"), "simulated_data.Rdump")
