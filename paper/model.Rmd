...
saveRDS(femfit, file = "female_scaled_phenology.rds")
posterior <- as.array(femfit)
post_re <- extract.samples(femfit)
prior_re <- extract.prior(femfit)
posterior <- as.array(femfit)
np <- nuts_params(femfit)
fempars1 <- precis(femfit)

hist(post_re$h50[,,1])
hist(post_re$h50[,,2])

betas <- fempars[str_detect(rownames(fempars), "beta"),]
alphas <- fempars[str_detect(rownames(fempars), "^a_"),]
divergences <- filter(np, Parameter=="divergent__")

mcmc_areas(posterior, regex_pars = c("beta_s", "beta_p", "beta$", "nu"))
...
mcmc_pairs(posterior, np=np, pars=c("beta", "prov_sigma", "site_sigma", "clone_sigma", "site_nu", "prov_nu", "kappa[1]", "kappa[2]"))
```
...
femtest <- stan("phenology_corr.stan",
                chains=4, warmup=2000, iter=2500, data=fdf)

```

## What about an error model?
...
posterior <- as.array(femfit)
post_re <- extract.samples(femtest)
...
slopedraft <- ulam(
    alist(
     ...
     data=mdf, sample=FALSE, declare_all_data = FALSE)
...
mfit <- stan("slopes.stan",
                chains=4, cores=4, warmup=3000, iter=5000, data=mdf)
```
