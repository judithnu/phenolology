...
head(phendf)
```
```{r ulam version sex only no corr, eval=FALSE}
...
``` {r ulam version sex with corr, eval=FALSE}
...
```{r sample from prior, eval=FALSE}
fit_full_draft <- ulam(
    alist(
        Phenophase_Derived ~ dordlogit(phi, kappa),
        phi <- alphas + beta*forcing_accum,
    ...
    data=phendf, sample=FALSE, declare_all_data= FALSE, sample_prior = TRUE
)
```