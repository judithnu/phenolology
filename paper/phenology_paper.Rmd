...
Average predictive comparisons for each effect $u$ were calculated by calculating the absolute value of the difference between the expected value at a given data point $n$ (where $n$ were all uniquely occuring combinations of site, provenance, clone, and year in the phenology dataset) for the input variable of interest $u^{(k)}$ and the other input variables $v$ and that datapoint for its actual value of $u$, mulitiplying that difference by a weight $\omega$ that represents the similarity in $v$ between a datapoint with $u_i$ versus $u^{(k)}$. The weighted average of these differences is taken over all data points, all values of u, and samples from the posterior distribution. 
...
7000 random samples from each model fit in Stan were used to calculate average predictive comparisons.
...
Receptivity requires more forcing to begin and less to end than pollen shed. Model estimates are tight.

![$fstart$ and $fend$ model estimates](../../phd/LabNotebook/images/2019-11-14_fstartfenddata.png)
...
Differences in clone or year cause th larger changes in $fstart$ than site or provenance \ref{apcforce}.
