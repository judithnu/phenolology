...
The model was fit in the probabilistic programming language Stan (vers. 2.19.1 via RStan [@standevelopmentteam2019]) which uses the No-U-Turn Sampler, an efficient Markov Chain Monte Carlo method that extends the Hamiltonian Monte Carlo algorithm [@carpenter2017], to sample the joint posterior.

Sampling efficiency for the year cluster was improved with a non-centered parameterization. I ran 8 independent chains for 1500 iterations, discarding 700 warm-up iterations for a total of 6400 posterior samples for each parameter.
...
Average predictive comparisons allow parameters in non-linear models to be compared like regression coefficients in linear models. Using $f_start$ as the expected value, average predictive comparisons were calculated for site, provenance, clone, and year following @gelman2007b. The average predictive comparison for forcing units was calculated with phenological state as the expected value. [link to code supplement] 
