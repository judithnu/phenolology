...
### Actual methods
I modeled the derived phenological states with a Bayesian multilevel ordered logistic model fit in Stan with the no-u-turn sampler [@standevelopmentteamRStanInterfaceStan2018]. A cumulative link function accounts for the ordering of phenological states and relates phenological states to a linear model. This type of model makes no assumption about the distance between phenological states and estimates intercepts (typically called "cutpoints" in this type of model) that separate each state. 

The model was fit separately to data for male strobili and female strobili.

The probability of being in state $s \in \{1,2,3\}$ is

$$\text{OrderedLogistic}(s~|~\eta,c) = \left\{ \begin{array}{ll} 1 -
\text{logistic}(\eta - \kappa_1)  &  \text{if } s = 1 \\[4pt]
\text{logistic}(\eta - \kappa_1) - \text{logistic}(\eta - \kappa_2)  &
\text{if } s=2  \\[4pt] \text{logistic}(\eta -
\kappa_3) - 0  &  \text{if } s = 3 \end{array} \right.$$

where the cutpoints $\kappa_s < \kappa_{s+1}$ and $\eta$ is a linear model relating forcing units to phenological state.
