...
Seed orchards in British Columbia produce large amounts of tree seed for reforestation. Interior lodgepole pine (*Pinus contorta* var *latifolia*) is planted more than any other species at rates of [XXX] million/year as of [201X]. In the late 90s, it became apparent that seed yields (filled seeds per cone) at seed orchards in the northern Okanagan were much lower than in orchards near Prince George. To plan for future seed production and orchard establishment and management, studies were undertaken to understand why seed yields at north Okanagan orchards were lower than at the Prince George Tree Improvement Station orchards. Many orchards were already collecting at least some phenological data in order to rate seedlots [@Woods1996], but standardized collection across all orchards began in 2006 under the Forest Genetics Council's Operational Tree Improvement Program 0722 [@Webber2007]. Pollination phenology data recorded as part of this program included pollen concentrations and dates of pollen shed and cone receptivity. 
...
In the first stage, I will model pollination phenology.
...
![Map of seed orchard locations. Boxed area in map on left is shown in greater detail on the right.](~/Documents/research_phd/maps/SiteMaps/sitemap.png)
...
### Actual methods
I modeled the derived phenological states with a Bayesian multilevel ordered logistic model fit in Stan [@standevelopmentteamRStanInterfaceStan2018].

For phenological states $k \in \{1,2,3\}$, the probability of 

The ordered logistic probability mass function is
$$\text{OrderedLogistic}(k~|~\eta,c) = \left\{ \begin{array}{ll} 1 -
\text{logistic}(\eta - c_1)  &  \text{if } k = 1, \\[4pt]
\text{logistic}(\eta - c_1) - \text{logistic}(\eta - c_2)  &
\text{if } k=2, \text{and} \\[4pt] \text{logistic}(\eta -
c_3) - 0  &  \text{if } k = 3. \end{array} \right.$$


$$\begin{array}{rlr}
S_i & \sim OrderedLogistic(\eta,\kappa) & \text{probability of data}\\
\eta_i & = \beta_{sex} h_i + \alpha_{provenance} + \alpha_{clone} & \text{linear model}\\
\kappa_k & \sim \mathrm{normal}(5,10) & \text{common prior for cutpoints}\\
\beta_{clone} & \sim \mathrm{beta}(1,1) & \text{prior for transition speed}\\
\alpha_{clone} & \sim \mathrm{normal}(0,1) & \text{prior for clone threshold effects}\\
\alpha_{provenance} & \sim \mathrm{normal}(0,1) & \text{prior for provenance threshold effects}
\end{array}$$


The overall threshold for for phase transition can be calculated by dividing the slope by the intercept.

$$\begin{array}{threshold}
\bf{H}_1 &= \beta_{sex}/(c_1+\alpha_{clone}+\alpha_{provenance})\\
\bf{H}_2 &= \beta_{sex}/(c_2+\alpha_{clone}+\alpha_{provenance})
\end{array}$$
...
\\
\kappa_k&\sim \mathrm{normal}(5,10) &\text{common prior for cutpoints}\\
\beta_{clone}&\sim \mathrm{beta}(1,1) \\
\alpha_{clone} &\sim \mathrm{normal}(0,1) \\
\alpha_{provenance} &\sim \mathrm{normal}(0,1) \\
\bf{H}_1 &= \beta_{sex}/(c_1+\alpha_{clone}+\alpha_{provenance})\\
\bf{H}_2 &= \beta_{sex}/(c_2+\alpha_{clone}+\alpha_{provenance})
\end{array}$$
