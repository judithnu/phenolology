...
Average predictive comparisons for each effect $u$ were calculated by calculating the squared difference between the expected value at a given data point (where datapoints were all uniquely occuring combinations of site, provenance, clone, and year in the phenology dataset) $n$ for the input variable of interest $u^{(k)}$ and the other input variables $v$ and that datapoint for its actual value of $u$, mulitiplying that squared difference by a weight $\omega$ that represents the similarity in $v$ between a datapoint with $u_i$ versus $u^{(k)}$. The weighted average of these squared differences is taken over all datapoints, values of u, and samples. [IS THIS SENTENCE REALLY TRUE?]

$$\hat \Delta_u = \sqrt{(\frac{\sum_{i=1}^{n}\sum_{k=1}^{K}\sum_{s=1}^S[\sum_{j\in\{k\}}\omega_{ij}](\mathrm{E}(y|u^{(k)},v_i, \theta^{(s)})-\mathrm{E}(y|u_i,v_i, \theta^{(s)}))^2}
...
Do site, clone, prov, year effects differ between male and females? See apcs to begin
